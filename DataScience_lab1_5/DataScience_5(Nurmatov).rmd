...
```{r}
ass_rules <- apriori(table2, parameter = list(support = 0.01, confidence = 0.5))
summary(ass_rules)
```
...
```{r}
plot(ass_rules, measure = c("support", "confidence"), shading = "lift")
```
...
```{r}
subset1 <- table2[1:8000]
subset2 <- table2[8000:10000]
...
```{r}
train_apr <- apriori(subset1, parameter = list(supp = 0.01, conf = 0.8))
test_apr <- apriori(subset2, parameter = list(supp = 0.009, conf = 0.75))

summary(train_apr)
summary(test_apr)

intersectRules <- intersect(train_apr, test_apr)
